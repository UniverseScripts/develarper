...
async def test_integration_end_to_end(tmp_path) -> None:  # type: ignore[type-arg]
    ...
    with patch("engines.remote_llm.RemoteLLMEngine.generate", new_callable=AsyncMock) as mock_remote, \
    ...
    for tid in ["factual_1", "sentiment_1", "summarization_1", "ner_1",
                "debug_1", "logic_1", "code_gen_1"]:
